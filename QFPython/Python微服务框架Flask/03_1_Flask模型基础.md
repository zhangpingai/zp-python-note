...
查询数据
    ...
    示例:   
         查询:
            ...
            persons = Person.query.filter(Person.name.startswith('宝'))  # 开头匹配
            persons = Person.query.filter(Person.name.endswith('宝'))  # 结尾匹配
            persons = Person.query.filter(Person.name.contains('宝'))  # 包含
            persons = Person.query.filter(Person.name.in_([11,12,22]))  # in_
